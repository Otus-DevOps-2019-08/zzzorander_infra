...
# Ansible-2
## HW
- Рассмотрели вариант использования тегов в плейбуке, сделали плейбук "все в одном" с разделением тегами. По пути посмотрели мельком как работать с шаблонами Jinja2, поняли как работают handlers
- Заменили теги в сценарии на группы хостов, к которым разные части сыенария применяются. Дописали сценарий для деплоя сами.
- Поделили наш плейбук на несколько, чтобы на каждую роль был свой плейбук + общий деплой, переименовали старые плейбуки. Сделали playbook site.yml который делает import_playbook для всех playbook'ов, учавствующих в развертывани.
- Переписали provision для packer. Для этого создали два плейбука `packer_app.yml`  и `packer_db.yml` и изменили соответствующим образом `db.json` и `app.json`, после чего раскатали новые образы.
- Проверили работоспособность приложения - все в порядке.
